...
static void from_json(const json& j, Config::Visuals& v)
{
    ...
    read<value_t::string>(j, "customSkybox", v.customSkybox);
...
static void from_json(const json& j, Config::Misc& m)
{
    ...
    read<value_t::object>(j, "Draw aimbot FOV", m.drawAimbotFov);
    ...
    read<value_t::string>(j, "Custom Name", m.customName);
    ...
    read<value_t::string>(j, "Fake Item Player Name", m.fakeItemPlayerName);
    read<value_t::string>(j, "Fake Item Skin Name", m.fakeItemName);
