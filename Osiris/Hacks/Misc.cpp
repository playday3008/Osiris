...
{
    if (config->misc.playerBlocker && config->misc.playerBlockerKey.isDown()) {
...
void Misc::fakeItem(bool set) noexcept
{
    ...
    if (shouldSet == 1)
    {
        ...
        if (interfaces->engine->isInGame() && changeName(false, std::string{ "\n \x1\xB" }
            ...
            .append("• • ")
            .append(team)
            .append(config->misc.fakeItemPlayerName)
            .append(config->misc.fakeItemMessageType == 0 ? "\x01 has opened a container and found: \x1\xB" : "\x01 has recieved in trade: \x1\xB")
            .append(color)
            .append(star)
            .append(stattrak)
            .append(item)
