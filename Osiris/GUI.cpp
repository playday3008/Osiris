...
void GUI::renderTriggerbotWindow(bool contentOnly) noexcept
{
    ...
    ImGui::SliderFloat("Max aim inaccuracy", &config->triggerbot[currentWeapon].maxAimInaccuracy, 0.0f, 1.0f, "%.5f", ImGuiSliderFlags_AlwaysClamp | ImGuiSliderFlags_Logarithmic);
    ImGui::SliderFloat("Max shot inaccuracy", &config->triggerbot[currentWeapon].maxShotInaccuracy, 0.0f, 1.0f, "%.5f", ImGuiSliderFlags_AlwaysClamp | ImGuiSliderFlags_Logarithmic);
...
void GUI::renderVisualsWindow(bool contentOnly) noexcept
{
    ...
    if (config->visuals.ragdollTimescale) {
        ImGui::PushItemWidth(280.0f);
        ImGui::PushID("Physics timescale value");
...
void GUI::renderMiscWindow(bool contentOnly) noexcept
{
    ...
    ImGuiCustom::colorPicker("Draw aimbot FOV", config->misc.drawAimbotFov);
    ...
    ImGui::PopID();
    ImGui::Checkbox("Ping Based Choked packets", &config->misc.pingBasedChoked);
    /*
    ...
    */
    ...
    ImGui::SameLine();
    ImGui::PushID("Player Blocker Key");
    hotkey2("", config->misc.playerBlockerKey);
    ImGui::PopID();
